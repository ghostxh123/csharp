...
namespace SGsystem.share
{
    class Validator
    {
        ...
        {
           // return Regex.IsMatch(strIn, @"^(0|[1-9]\d*)$");
            return Regex.IsMatch(strIn, @"^[0-9]\d{0,10}$");
        ...
        //验证是否为正数或两位小数
        static public Boolean IsNumber2(String strIn)
        {
            return Regex.IsMatch(strIn, @"^[0-9]+(\.[0-9]{0,2})?$");
        }
